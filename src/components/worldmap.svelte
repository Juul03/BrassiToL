<script>
  ...
  onMount(async () => {
    ...
    const unsubscribe = selectedTaxonomyStore.subscribe((value) => {
        ...
        countryNamesArray = countryNamesArray.flat();
  ...
  function colorCountry(countryNames) {
    // Reset all countries to green
    d3.select("#content g.map")
        .selectAll("path")
        .style("fill", "green");

    // Apply the orange color to the selected countries
    d3.select("#content g.map")
        .selectAll("path")
        .filter((d) => countryNames.includes(d.properties.name))
        .style("fill", "orange");
}
