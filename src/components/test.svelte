<script>
  ...
  onMount(() => {
    // Find and remove nodes labeled "ant" and "dog" from the parsedData
    const updatedData = removeNodesFromTree(parsedData, ["S1384", "MYZV"]);

    // Create the phylogenetic tree with the updated data
    const svg = createPhylogeneticTree(updatedData);
  ...
  const removeNodesFromTree = (data, nodesToRemove) => {
    ...
    const updatedData = JSON.parse(JSON.stringify(data)); // Create a deep copy
